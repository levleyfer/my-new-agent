...
export default function HistoryPage() {
  ...
  function statusBadge(s: any) {
    if (s.status === "completed" && s.overall_score !== null) {
      const color = s.overall_score >= 75 ? "text-emerald-600 dark:text-emerald-400" : s.overall_score >= 55 ? "text-amber-600 dark:text-amber-400" : "text-red-500 dark:text-red-400";
    ...
    if (s.status === "processing") return (
      <span className="flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-900/30 border border-amber-100 dark:border-amber-900/50 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">
        <Clock size={10} className="animate-spin" style={{ animationDuration: "2s" }} />{T.badge_processing}
      </span>
    );
    if (s.status === "failed") return (
      <span className="rounded-full bg-red-50 dark:bg-red-900/30 border border-red-100 dark:border-red-900/50 px-2.5 py-0.5 text-xs font-medium text-red-600 dark:text-red-400">{T.badge_failed}</span>
    );
    return (
      <span className="rounded-full bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 px-2.5 py-0.5 text-xs font-medium text-gray-500 dark:text-gray-400">{T.badge_pending}</span>
    );
  }
  ...
  return (
    <div className="fade-in max-w-3xl">
      ...
      {isLoading ? (
        <div className="space-y-2.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-card">
              ...
              <div className="flex gap-2"><Skeleton className="h-5 w-20 rounded-full" /><Skeleton className="h-4 w-24 rounded-full" /></div>
            </div>
          ))}
        </div>
      ) : isError ? (
        ...
        <EmptyState icon={Mic} title={T.empty_hist_title} description={T.empty_hist_desc}
        ...
        <>
              ...
              <Link key={s.id} to={`/sessions/${s.id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1.5 flex items-center gap-2">
                    <span className="rounded-full border border-indigo-100 dark:border-indigo-900/50 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 text-[11px] font-medium text-indigo-700 dark:text-indigo-400 capitalize">{s.question_category}</span>
                ...
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <ChevronRight size={15} className="text-gray-300 dark:text-gray-600 transition-colors group-hover:text-indigo-400 rtl:rotate-180" />
                </div>
              </Link>
            ))}
          </div>
          ...
          {totalPages > 1 && (
            <div className="mt-6 flex items-center justify-center gap-2">
              ...
              <span className="px-3 text-sm text-gray-500 dark:text-gray-400">{page + 1} / {totalPages}</span>
