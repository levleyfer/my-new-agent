...
export default function PracticePage() {
  ...
  const selectCls =
    'rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3.5 py-2 text-sm text-gray-700 dark:text-gray-200 shadow-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900/40 transition-all';
    ...
    return (
      <div className="fade-in max-w-3xl">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2.5">
          ...
          <button
            onClick={pickRandom}
            className="ms-auto flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 transition-colors"
          >
            <Shuffle size={14} />
            {T.btn_random}
          </button>
        ...
        {error && (
          <div className="mb-4 rounded-xl border border-red-100 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-400">
            {error}
          </div>
        )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2.5 flex gap-2">
                      <span className="rounded-full border border-indigo-100 dark:border-indigo-900/50 bg-indigo-50 dark:bg-indigo-900/30 px-2.5 py-0.5 text-[11px] font-medium text-indigo-700 dark:text-indigo-400 capitalize">
                        {q.category}
                      </span>
                      <span
                        className={`rounded-full border px-2.5 py-0.5 text-[11px] font-medium capitalize ${DIFF_COLORS[q.difficulty] ?? 'bg-gray-50 text-gray-600 border-gray-100 dark:bg-gray-700 dark:text-gray-400 dark:border-gray-600'}`}
                      >
                        {q.difficulty}
                      </span>
                    </div>
                  ...
                  <ChevronRight
                    size={17}
                    className="mt-0.5 shrink-0 text-gray-300 dark:text-gray-600 transition-colors group-hover:text-indigo-400 rtl:rotate-180"
                  />
                </div>
              </button>
            ))}
  ...
  return (
    <div className="fade-in max-w-2xl">
      <button
        ...
        className="mb-6 flex items-center gap-1.5 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white transition-colors"
      >
      ...
      <div className="mb-6 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-900/20 dark:to-violet-900/20 p-5">
        <div className="mb-3 flex gap-2">
          <span className="rounded-full border border-indigo-100 dark:border-indigo-900/50 bg-white/60 dark:bg-gray-800/60 px-2.5 py-0.5 text-[11px] font-medium text-indigo-700 dark:text-indigo-400 capitalize">
            {selectedQuestion?.category}
          </span>
          <span
            className={`rounded-full border px-2.5 py-0.5 text-[11px] font-medium capitalize ${DIFF_COLORS[selectedQuestion?.difficulty] ?? ''}`}
          >
      ...
      {error && (
        <div className="mb-5 rounded-xl border border-red-100 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-400">
          {error}
        </div>
      )}
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-8 shadow-card flex flex-col items-center gap-6">
        <div className="text-center">
          ...
          {elapsed > 150 && recordingState === 'recording' && (
        ...
        {recordingState === 'idle' && (
          <button
            ...
            className="group flex h-16 w-16 items-center justify-center rounded-full bg-red-500 text-white shadow-lg shadow-red-200 dark:shadow-red-900/40 transition-all hover:scale-105 hover:bg-red-600 active:scale-95"
          >
            <Mic size={26} />
          </button>
        )}
        ...
        {recordingState === 'recording' && (
          <div className="flex flex-col items-center gap-3">
            ...
            <button
              ...
              className="flex h-16 w-16 items-center justify-center rounded-full bg-gray-900 dark:bg-gray-700 text-white shadow-lg transition-all hover:scale-105 hover:bg-gray-800 dark:hover:bg-gray-600 active:scale-95"
            >
              <MicOff size={24} />
            </button>
