...
export default function SessionPage() {
  ...
  const backLink = (
    <Link
      to="/history"
      className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white transition-colors"
    >
  ...
  if (isError) {
    return (
      <div className="max-w-3xl">
        {backLink}
        <div className="mt-6">
          <ErrorState message={T.err_session} onRetry={() => refetch()} />
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl fade-in">
      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        {status === 'completed' && (
          <div className="flex items-center gap-2">
            {question?.id && (
              <Link
                to={`/practice?question_id=${question.id}`}
      ...
      <div className="mb-6 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-900/20 dark:to-violet-900/20 p-5">
        <div className="mb-2.5 flex gap-2">
          <span className="rounded-full border border-indigo-100 dark:border-indigo-900/50 bg-white/70 dark:bg-gray-800/60 px-2.5 py-0.5 text-[11px] font-medium text-indigo-700 dark:text-indigo-400 capitalize">
            {question?.category}
          </span>
          <span className="rounded-full border border-gray-200 dark:border-gray-600 bg-white/70 dark:bg-gray-800/60 px-2.5 py-0.5 text-[11px] font-medium text-gray-600 dark:text-gray-400 capitalize">
            {question?.difficulty}
          </span>
        </div>
      ...
      {status === 'completed' && analysis && (
        <>
          ...
          <div ref={resultsRef}>
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
                ...
                <div className="flex-1 w-full">
                  ...
                  <ScoreBar
                    ...
                    feedback={analysis.clarity_feedback}
                  />
                  <ScoreBar
                    label={T.dim_completeness}
                    score={analysis.completeness_score}
                    feedback={analysis.completeness_feedback}
                  />
                  <ScoreBar
                    label={T.dim_communication}
            ...
            <div className="mb-5 rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-card">
              <ScoreBar
                label={T.dim_confidence}
                score={analysis.confidence_score}
                feedback={analysis.confidence_feedback}
              />
              <ScoreBar
                label={T.dim_star}
                score={analysis.star_method_score}
                feedback={analysis.star_method_feedback}
              />
              <ScoreBar
                label={T.dim_technical}
                score={analysis.technical_depth_score}
                feedback={analysis.technical_depth_feedback}
              />
            </div>
            ...
            {analysis.filler_words?.length > 0 && (
              <div className="mb-5 rounded-2xl border border-amber-100 dark:border-amber-900/50 bg-white dark:bg-gray-800 p-5 shadow-card">
                ...
                <div className="flex flex-wrap gap-2">
                  {analysis.filler_words.map((fw: any, i: number) => (
                    <Badge key={i} word={fw.word} count={fw.count} />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="mb-5 grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-emerald-100 dark:border-emerald-900/50 bg-emerald-50 dark:bg-emerald-900/20 p-5">
                <h2 className="mb-3 text-sm font-semibold text-emerald-800 dark:text-emerald-300">
                  {T.strengths_title}
                </h2>
                <ul className="space-y-2">
                  {analysis.strengths.map((s: string, i: number) => (
                    <li
                      key={i}
                      className="flex gap-2 text-sm text-emerald-700 dark:text-emerald-400"
                    >
              ...
              <div className="rounded-2xl border border-red-100 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 p-5">
                <h2 className="mb-3 text-sm font-semibold text-red-800 dark:text-red-300">
                  {T.weaknesses_title}
                </h2>
                <ul className="space-y-2">
              ...
              <div className="mb-5 rounded-2xl border border-violet-100 dark:border-violet-900/50 bg-violet-50 dark:bg-violet-900/20 p-5">
                ...
                <div className="space-y-2.5">
                  {question.follow_ups.map((fq: string, i: number) => (
                    <div
                      key={i}
                      className="flex gap-2.5 text-sm text-violet-700 dark:text-violet-400"
                    >
                      <span className="mt-0.5 shrink-0 font-bold text-violet-400 dark:text-violet-500">
                        Q{i + 1}.
                      </span>
                      <p>{fq}</p>
                    </div>
            ...
            {transcript && (
              <div className="rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-card">
                ...
                <p className="mb-3 text-xs text-gray-400 dark:text-gray-500">
                  {transcript.word_count} {T.unit_words} · {transcript.language.toUpperCase()}
                </p>
                <p className="text-sm leading-relaxed text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                  {transcript.text}
                </p>
