...
export default function AnalyticsPage() {
  ...
  } = useQuery({ queryKey: ['analytics-summary'], queryFn: getAnalyticsSummary, staleTime: 0 });
  const { data: progress, isLoading: progressLoading } = useQuery({
    queryKey: ['analytics-progress'],
    queryFn: getAnalyticsProgress,
    staleTime: 0,
  });
  const { data: weaknesses, isLoading: weaknessLoading } = useQuery({
    queryKey: ['analytics-weaknesses'],
    queryFn: getAnalyticsWeaknesses,
    staleTime: 0,
  });
  const { data: fillerWords, isLoading: fillerLoading } = useQuery({
    queryKey: ['analytics-filler-words'],
    queryFn: getAnalyticsFillerWords,
    staleTime: 0,
  });
  ...
  const catData = (categoryData?.categories ?? []).map((c: any) => ({
    ...c,
      ...
      <div className="mb-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
        {summaryLoading ? (
          ...
          <div className="col-span-full">
            <ErrorState message={T.err_analytics} onRetry={() => summaryRefetch()} />
          </div>
        ) : (
          <>
            <StatCard
              label={T.stat_avg_overall}
            ...
            />
            <StatCard
              label={T.stat_avg_comm}
              value={summary?.avg_communication_score ?? null}
              icon={MessageSquare}
              iconBg="bg-violet-50 dark:bg-violet-900/40"
              iconColor="text-violet-600 dark:text-violet-400"
            />
            <StatCard
              label={T.stat_completed_a}
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={catData} margin={{ top: 5, right: 16, left: -10, bottom: 5 }}>
              ...
              <Tooltip
                ...
                formatter={(v: number) => [`${v}/100`, T.axis_score]}
              />
        ...
        <div className="mb-5 rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-card">
          ...
          <div className="space-y-2.5">
            {weaknessData.map((w: any, i: number) => (
              <div
                key={i}
                className="flex items-start gap-3 rounded-xl bg-red-50 dark:bg-red-900/20 px-4 py-3"
              >
